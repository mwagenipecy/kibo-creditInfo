...
<div>
    ...
    @if(!empty($lenders))
        <ul class="absolute z-10 bg-white border border-gray-200 rounded-md mt-1 w-full shadow-lg">
            @forelse($lenders as $lender)
                <li class="px-4 py-2 hover:bg-gray-100 cursor-pointer">
                    {{ $lender->name }}
                </li>
            @empty
                <li class="px-4 py-2 text-gray-500">No lenders found</li>
            @endforelse
        </ul>
    @endif
</div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($documents as $document)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $document->lender->name ?? 'N/A' }}
                                         </td>
