                    ...
                    <div class="mt-8 p-6 bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl border border-blue-200">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            @foreach(['front', 'side', 'back'] as $view)
                                @if(isset($existingImages[$view]) && is_object($existingImages[$view]) && isset($existingImages[$view]->image_url))
                                    <div class="space-y-2">
                                        <h5 class="text-xs font-medium text-gray-600 capitalize">{{ $view }} View</h5>
                                        <div class="relative group">
                                          
                                        <img 
                                                src="{{ asset('storage/' . $existingImages[$view]->image_url) }}"
                        ...
                        @if(isset($existingImages['additional']) && is_array($existingImages['additional']) && count($existingImages['additional']) > 0)
                            <div class="mt-6">
                                ...
                                <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
                                    @foreach($existingImages['additional'] as $image)
                                        @if(is_object($image) && isset($image->image_url) && isset($image->id))
                                            <div class="relative group">
                                            <img 
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">
                                Are you sure you want to mark this vehicle as deleted? This action will change the vehicle status to "deleted" but will not permanently remove the vehicle from the database.
                            </p>
                            <div class="mt-4 bg-yellow-50 border border-yellow-200 rounded-md p-3">
                                <div class="flex">
                                    ...
                                    <div class="text-sm text-yellow-700">
                                        <strong>Note:</strong> The vehicle will be marked as deleted but can be restored by changing its status back to active.
                                    </div>
                                </div>
                            </div>
                        </div>
