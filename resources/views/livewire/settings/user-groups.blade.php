<div>
            ...
            <li class="mr-2" wire:click="setView(2)">
                <a href="#" class="inline-flex p-4 rounded-t-lg group
                    ...
                    @else 
                        border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300
                    @endif"
                    @if($selected == 2) aria-current="page" @endif>
                    <svg aria-hidden="true" class="w-5 h-5 mr-2 
                        @if($selected == 2) 
                            text-green-600 dark:text-green-500
                        @else 
            ...
            <li class="mr-2" wire:click="setView(3)">
                <a href="#" class="inline-flex p-4 rounded-t-lg group
                    ...
                    @else 
                        border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300
                    @endif"
                    @if($selected == 3) aria-current="page" @endif>
                    <svg aria-hidden="true" class="w-5 h-5 mr-2 
                        @if($selected == 3) 
                            text-green-600 dark:text-green-500
                        @else 
    ...
    <div class="bg-gray-100 p-2 rounded-2xl mt-4">
        <!-- Success Message (Shared across tabs) -->
        ...
        <!-- Roles List Tab Content -->
        @if($selected == 1)
            <div class="w-full bg-white p-4 sm:p-6 overflow-hidden rounded-2xl shadow-md shadow-gray-200">
                <h5 class="text-lg font-semibold text-gray-700 mb-4">ROLES LIST</h5>
                <livewire:settings.department-list/>
            </div>
        @endif

        <!-- Create Roles Tab Content -->
        @if($selected == 2)
            <div class="w-full bg-white p-4 sm:p-6 overflow-hidden rounded-2xl shadow-md shadow-gray-200">
                <h5 class="text-lg font-semibold text-gray-700 mb-4">CREATE A NEW ROLE</h5>
                <livewire:settings.create-role/>
            </div>
        @endif

        <!-- Edit Roles Tab Content -->
        @if($selected == 3)
            <div class="w-full bg-white p-4 sm:p-6 overflow-hidden rounded-2xl shadow-md shadow-gray-200">
                <h5 class="text-lg font-semibold text-gray-700 mb-4">EDIT ROLES</h5>
