...
class LoansTable extends LivewireDatatable
{
    ...
    public function approve($id)
    {
        ...
        if ($loanPaymentMethod->pay_method == 'MOBILE' || $loanPaymentMethod->pay_method == 'BANK') {
            ...
            $loanAmount = (float) ClientsModel::where('id', $loanPaymentMethod->client_id)->value('amount');
    ...
    public function topUp($id)
    {
        ...
        if ($client_total_amount >= ($total_principle + $total_interest)) {
            ...
            $client_new_balance = (float) $client_total_amount - (float) $total_principle;
            ...
            $principle_collection_account_new_balance = (float) $total_principle + (float) $principle_prev_balance;
            // update balance
            ...
            // debit
            $record_on_general_ledger->debit($client_account_number, $client_new_balance, $principle_account_number, $total_principle, 'Loan top up principle transaction', $loan->loan_id);
            ...
            // interest transactions
            $client_new_balance = $client_new_balance - (float) $total_interest;
            // update amount
            AccountsModel::where('account_number', $client_account_number)->update(['balance' => $client_new_balance]);

            // credit section with interest
            $interest_collection_new_balance = (float) $interest_prev_balance + (float) $total_interest;
            // update balance
            AccountsModel::where('account_number', $interest_account_number)->update(['balance' => $interest_collection_new_balance]);

            // debit
            $record_on_general_ledger->debit($client_account_number, $client_new_balance, $interest_account_number, $total_interest, 'Loan top up interest transaction', $loan->loan_id);
    ...
    public function processPayment()
    {
        ...
        // debit
        $savings_ledger_account_prev_balance = (float) AccountsModel::where('account_number', $this->bank1)->value('balance');
        $savings_ledger_account_new_balance = (float) ($savings_ledger_account_prev_balance - $this->principle);
        AccountsModel::where('account_number', $this->bank1)->update(['balance' => $savings_ledger_account_new_balance]);

        // credit
        $savings_account_new_balance = (float) AccountsModel::where('account_number', $this->loan_account_number)->value('balance') + (float) $this->principle;
