...
class TellerPosition extends Component
{
    ...
    public function approveModal($id)
    {
        ...
        if ($this->status == 'OVER') {
            $this->overAmount = (float) $this->cash - (float) $this->til_balance;
        } elseif ($this->status == 'UNDER') {
            $this->underAmount = -((float) $this->cash - (float) $this->til_balance);
        }
    ...
    public function closeTil()
    {
        ...
        if (Hash::check($this->password, auth()->user()->password)) {
            ...
            if ($this->status == 'OVER') {
                ...
                $teller_new_balance = (float) $tellerData->balance - (float) $amount_required;
                ...
                $destination_new_balance = (float) $destinationAccountData->balance + (float) $amount_required;
                ...
                $record_on_generalLG->credit(
                    $destination_account_number, $destination_new_balance, $teller_account_number, $amount_required, 'transfer of over amount', '');

                // updte teller end of day table
                TellerEndOfDayPositions::where('id', $this->selectedId)->update([
                    'status' => 'BALANCED',
                    'tiller_cash_at_hand' => (float) $this->cash - (float) $amount_required,
                ]);
                ...
                if ($member_account->balance >= $amount_required) {
                    ...
                    $member_new_balance = (float) $member_account->balance - (float) $amount_required;
                    ...
                    $strong_room_new_balance = (float) $strongRoom->balance + (float) $amount_required;
                    ...
                    $record_on_general_ledger->credit(
                        $strongRoom->account_number, $strong_room_new_balance, $member_account->account_number, $amount_required, 'force close til', '');
                    ...
                    // credit strong room accounts
                    $strongRoom = AccountsModel::where('sub_category_code', 1025)->first();
                    $strong_room_new_balance = (float) $strongRoom->balance + (float) $member_account->balance;
                    // update strong room account
                    AccountsModel::where('sub_category_code', 1025)->update(['balance' => $strong_room_new_balance]);
                    ...
                    // record on credit
                    $record_on_general_ledger->credit(
                        $strongRoom->account_number, $strong_room_new_balance, $member_account->account_number, $member_account->balance, 'force close til', ''
                    );

                    // updte teller end of day table
                    TellerEndOfDayPositions::where('id', $this->selectedId)->update([
                        'status' => 'UNDER',
                        'tiller_cash_at_hand' => (float) $this->cash + (float) $member_account->balance,
                    ]);
                    session()->flash('message', 'successfully');
                    $this->emit('refreshTellerPosition');
                }
