...
XML;
    ...
    public function render()
    {
        ...
        $this->monthly_sales = (float) $this->daily_sales * (float) $this->days_in_a_month;

        $this->gross_profit = $this->monthly_sales - (float) $this->cost_of_goods_sold;
        $this->net_profit = $this->gross_profit - (float) $this->monthly_taxes;
        $this->available_funds = ($this->net_profit - (float) $this->other_expenses) / 2;
        ...
        if ($this->future_interests) {
            ...
            if ($this->future_interests == 'YES') {
                ...
                if ($balance >= $total_principle_amount) {
                    ...
                    $client_new_balance = (float) $balance - (float) $total_principle_amount;
                    ...
                    $principle_collection_account_new_balance = (float) $total_principle_amount + (float) $principle_collection_prev_balance;
                    // update balance
                    ...
                    // debit
                    $record_on_general_ledger->debit($debit_account_number, $client_new_balance, $principle_collection_account_number, $total_principle_amount, 'loan repayment on close loan option', $loan_data->loan_id);
                ...
                $total_amount = (float) $total_interest_amount + (float) $total_principle_amount;
                ...
                if ($balance >= $total_amount) {
                    ...
                    $record_on_general_ledger = new general_ledger;
                    // debit
                    $record_on_general_ledger->debit($debit_account_number, $client_new_balance, $principle_collection_account_number, $total_principle_amount, 'loan repayment on close loan  for principle amount', $loan_data->loan_id);
                    ...
                    $client_new_balance = $client_new_balance - (float) $total_interest_amount;
                    // update amount
                    AccountsModel::where('account_number', $debit_account_number)->update(['balance' => $client_new_balance]);

                    // credit section with interest
                    $interest_collection_new_balance = (float) $interest_collection_prev_balance + (float) $total_interest_amount;
                    // update balance
                    AccountsModel::where('account_number', $interest_collection_account_number)->update(['balance' => $interest_collection_new_balance]);

                    // debit
                    $record_on_general_ledger->debit($debit_account_number, $client_new_balance, $interest_collection_account_number, $total_interest_amount, 'loan repayment on close loan for interest amount', $loan_data->loan_id);
    ...
    public function calc_principal($payno, $int, $pmt)
    {
    ...
    public function calc_number($pv, $int, $pmt)
    {
        // check that required values have been supplied
        if (empty($pv)) {
    ...
    public function calc_rate($pv, $payno, $pmt)
    {
        // check that required values have been supplied
        if (empty($pv)) {
            echo "<p class='error'>a value for PRINCIPAL is required</p>";
    ...
    public function print_schedulex($balance, $rate, $payment, $totPayment, $totInterest, $totPrincipal)
    {
        // check that required values have been supplied
        if (empty($balance)) {
            echo "<p class='error'>a value for PRINCIPAL is required</p>";
