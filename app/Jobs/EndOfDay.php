...
class EndOfDay implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            $this->loanPaymentWithNoArrears();
            $this->clientMenu();

        } catch (\Exception $e) {
            // dd($e->getMessage());
            Log::info('fails to run job');
        }

        // Mail::to('percyegno@gmail.com')->send('hello over there')->subject('');
    }
    ...
    public function loanRepayment($source_account_number, $destination_account_number, $interest_amount, $loan_id, $note, $id)
    {
        ...
        if ($source_prev_balance <= 0) {
            ...
            if ($interest_amount <= $source_prev_balance) {
                ...
                $new_paid_amount = (float) $prev_paid_mount + (float) ($interest_amount);
                loans_schedules::where('id', $id)->update(['payment' => $new_paid_amount]);

                $source_new_balance = (float) $source_prev_balance - (float) $interest_amount;
                ...
                //  paid amount
                $prev_paid_mount = $loanValues->payment;
                $new_paid_amount = (float) $prev_paid_mount + (float) ($source_prev_balance);
                loans_schedules::where('id', $id)->update(['payment' => $new_paid_amount]);
            ...
            $destination_new_balance = (float) $destination_acc->balane + (float) $interest_amount;
            ...
            $record_on_gl->credit($destination_account_number, $destination_new_balance, $source_account_number, $interest_amount, $note, $loan_id);
        }
